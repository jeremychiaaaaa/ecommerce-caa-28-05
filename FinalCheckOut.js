...
dispatch(setChangeAddress(false))
            } else{
                setLoading(false)
                setCartItems([])
            }
...
const viewAll = () => {
    ...
    nav.navigate('View Shipping Address')
 ...
 <Text style={{alignSelf:'center', fontWeight:'600', fontSize:22}}>Check Out</Text>
           ...
           <View style={{marginTop:15}}>
            <TouchableOpacity onPress={() => {
                ...
                <View style={{flexDirection:'row', alignItems:'center', }}>
                    <Text style={{fontSize:20, fontWeight: '600',marginLeft:20}}>Delivery address</Text>
          ...
          {address.length === 0 ?   <Text style={{fontWeight:'300', marginTop:10,marginLeft:20}}>No Address Saved Currently...</Text> : (
            ...
            <TouchableOpacity>
                <View style={{flexDirection:'row', alignItems:'center', borderBottomColor:'rgba(220,220,220,0.2)', paddingBottom:10, borderBottomWidth:1, marginTop:20 }}>
                    <Text style={{fontSize:20, fontWeight: '600',marginLeft:20}}>Delivery method</Text>
                    <Ionicons name='chevron-forward-outline' size={28} style={{position:'absolute', right:0, marginRight:10}} />
                </View>
            </TouchableOpacity>
            <View style={{marginTop:15}}>
                    <TouchableOpacity onPress={() => cardDetails.length === 0 ? nav.navigate('payment option') : nav.navigate('View Cards')}>
                <View style={{flexDirection:'row', alignItems:'center', borderBottomColor:'rgba(220,220,220,0.2)', paddingBottom:10, borderBottomWidth:1, }}>
                    <Text style={{fontSize:20, fontWeight: '600',marginLeft:20}}>Payment Method</Text>
                    <Ionicons name='chevron-forward-outline' size={28} style={{position:'absolute', right:0, marginRight:10}} />
                </View>
            </TouchableOpacity>
            {cards.length === 0 ?  <Text style={{fontWeight:'300', marginTop:10,marginLeft:20}}>No Cards Saved Currently...</Text> : (
                ...
                <View style={{marginTop:15,width:WIDTH*0.85,padding:10, justifyContent:'center', borderWidth:1, borderColor: 'rgba(220,220,220,0.3)', borderRadius:5, marginVertical:15,marginHorizontal:10}}
               
                
                >
                <View style={{flexDirection:'row', alignItems:'center'}}>
                   
                    <Text style={{textTransform:'uppercase', fontWeight:'600', fontSize:20, marginLeft:20}}>{i.type} ({i.number.slice(0,4)})</Text>
...
width:'100%', zIndex:10,alignSelf:'center', transform:[{translateY:-30}]
 ...
 <Text style={{alignSelf:'center', fontWeight:'600', fontSize:22}}>My Cart</Text>
        ...
        <View style={{ borderBottomColor:'rgba(220,220,220,0.2)', paddingVertical:10,borderBottomWidth: 1 }}>
            <Text style={{alignSelf:'center', fontWeight:'300', fontSize:18}}>{cartItems.length} items : Total (excluding deliery) <Text style={{fontWeight:'600',fontSize:20 }}>$ {Math.round(totalPrice * 100 ) / 100 }</Text></Text>
        </View>
        {final.map((i,index) => 
