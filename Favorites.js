            ...
            >
                 <TouchableOpacity style={{flexDirection:'row',  marginVertical:10}} onPress={() =>{ 
                  ...
                  <View style={{flexDirection:'row',alignItems:'center', }}>
                   ...
                   <TouchableOpacity style={{marginTop:15,width:WIDTH*0.27,borderWidth:1, borderColor:'green',}} 
                   onPress={() => {
                      setSelectedIndex(index)
...
export const ModalNavigator = ({sizee,favoriteItems,selectedIndex,setModalVisible,setColor,pickColor,color,pickSize}) => {
  const dispatch= useDispatch()
  const nav = useNavigation()
  const SelectSize = () => {
    return(
...
export const FavoritesStack = () => {
  ...
  return(
    <Stack.Navigator>
      <Stack.Screen name='Favoritee' component={Favorites} options={{
        headerShown:false
      }} />
      <Stack.Screen name='Modal Choose' component={ModalNavigator} options={{
        headerShown:false
      }} />
    </Stack.Navigator>
  )
